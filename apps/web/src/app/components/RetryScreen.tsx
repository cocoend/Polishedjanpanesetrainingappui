...
import { getRetryContext } from '../lib/api';
...
export default function RetryScreen({ onNavigate, activeSessionId }: RetryScreenProps) {
  const [retryContext, setRetryContext] = useState<RetryContextDto>({
  ...
  useEffect(() => {
    ...
    void getRetryContext(activeSessionId)
      .then((response) => {
        if (!isMounted) {
          return;
        }

        setRetryContext(response);
      })
      .catch(() => {
        // Keep the current prototype experience if the retry context is unavailable.
      });
  ...
  const firstFocusPoint = retryContext.focusPoints[0] ?? '前回の改善ポイントをもう一度意識してみましょう。';
  const secondFocusPoint = retryContext.focusPoints[1] ?? '最後に一文でまとめを入れてみましょう。';
  const firstExample = retryContext.conciseExamples[0] ?? '画面を指で触れて操作します';
  const secondExample =
    retryContext.conciseExamples[1] ?? 'このように、いつでも使える便利な道具です';
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-green-50 pb-8">
              ...
              <div className="flex-1">
                ...
                <div className="bg-white rounded-xl p-3 border border-red-100">
                  ...
                  <p className="text-gray-900 font-medium text-sm">
                    <span className="bg-green-200 px-1 rounded">{firstExample}</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="bg-white rounded-xl p-3 border border-blue-100">
                  ...
                  <p className="text-gray-900 font-medium text-sm">
                    <span className="bg-green-200 px-1 rounded">{secondExample}</span>
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="px-6 pb-6">
        ...
        <div className="bg-white border-2 border-gray-200 rounded-2xl p-5">
          <p className="text-gray-700 text-sm leading-relaxed">
            {retryContext.previousTranscriptText}
          </p>
        </div>
      </div>
