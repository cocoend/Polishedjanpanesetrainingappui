...
export default function TopicSelectionScreen({ onNavigate, onSelectTopic }: TopicSelectionScreenProps) {
  ...
  const [apiTopics, setApiTopics] = useState<Topic[]>(topics);
  ...
  useEffect(() => {
    ...
    void getThemes()
      .then((data) => {
        ...
        setApiTopics(data.map(mapThemeListItemToTopic));
      })
      .catch(() => {
        // Preserve the existing UI by falling back to the local prototype data.
      });
  ...
  const filteredTopics = apiTopics.filter(topic => {
