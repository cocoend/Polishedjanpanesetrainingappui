...
import type { LearnedCardDto } from '@polished/shared';

import { getLearnedCards } from '../lib/api';
...
function mapLearnedCardToRecord(card: LearnedCardDto): LearnedRecord {
  return {
    ...
    model: modelLabelMap[card.selectedModelId] ?? card.selectedModelId,
    score: card.latestScore,
    date: card.savedAt.slice(0, 10),
    attempts: card.attemptCount,
    improvement: card.improvementFromFirstScore,
    tags: card.purposeTags,
  };
}
...
export default function LearnedBoxScreen({ onNavigate, onMarkAsRead, onSelectRecord }: LearnedBoxScreenProps) {
  ...
  useEffect(() => {
    ...
    void getLearnedCards()
      .then((response) => {
        if (!isMounted) {
          return;
        }

        setRecords(response.cards.map(mapLearnedCardToRecord));
      })
      .catch(() => {
        // Preserve the prototype experience if the learned-card API is unavailable.
        if (isMounted) {
          setRecords(learnedRecords);
        }
      });
  ...
  const sourceRecords = records ?? learnedRecords;
  ...
  const averageScore =
    totalRecords > 0
      ? Math.round(sourceRecords.reduce((sum, r) => sum + r.score, 0) / totalRecords)
      : 0;
  const totalAttempts = sourceRecords.reduce((sum, r) => sum + r.attempts, 0);
